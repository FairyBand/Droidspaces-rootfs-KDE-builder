...
[Unit]
Description=Start KDE Plasma with direct KWin anland backend
...
# anland KWin 直连后端与 KDE Wayland 启动脚本
RUN apt-get update && \
    apt-get install -y --no-install-recommends \
        build-essential dpkg-dev fakeroot patch sudo git ca-certificates \
        dbus-bin && \
    git clone "$ANLAND_REPO" /tmp/anland && \
    cd /tmp/anland && \
    git checkout "$ANLAND_COMMIT" && \
    mkdir -p /opt/anland-kdefix && \
    cp -a kdefix/. /opt/anland-kdefix/ && \
    chmod +x /opt/anland-kdefix/build.sh && \
    WORKDIR=/tmp/anland-debbuild /opt/anland-kdefix/build.sh && \
    apt-get -f install -y && \
    printf '%s\n' "$ANLAND_COMMIT" > /opt/anland-kdefix/ANLAND_COMMIT && \
    apt-mark hold kwin-wayland xwayland || true && \
    rm -rf /tmp/anland /tmp/anland-debbuild /var/cache/apt/archives/*.deb && \
    apt-get clean && \
    rm -rf /var/lib/apt/lists/*
...
export KWIN_FORCE_SW_CURSOR="${KWIN_FORCE_SW_CURSOR:-1}"

find_kwin_wayland() {
    command -v kwin_wayland 2>/dev/null && return 0
    find /usr/bin /usr/lib /usr/libexec -type f -name kwin_wayland 2>/dev/null | head -n 1
}

echo "anland socket: $SOCK"
export ANLAND_SOCKET="$SOCK"

KWIN_WAYLAND="$(find_kwin_wayland)"
if [ -z "$KWIN_WAYLAND" ]; then
    echo "ERROR: kwin_wayland not found"
    exit 1
fi
export KWIN_WAYLAND

dbus-run-session -- bash -s <<'EOS'
set -euo pipefail

SESSION_BIN="$(command -v startplasma-waylandsession 2>/dev/null || true)"
if [ -z "$SESSION_BIN" ]; then
    for candidate in /usr/lib/*/libexec/startplasma-waylandsession /usr/libexec/startplasma-waylandsession; do
        if [ -x "$candidate" ]; then
            SESSION_BIN="$candidate"
            break
        fi
    done
fi
if [ -z "$SESSION_BIN" ]; then
    echo "ERROR: startplasma-waylandsession not found"
    exit 1
fi

echo "kwin wayland: $KWIN_WAYLAND"
echo "plasma session: $SESSION_BIN"
exec "$KWIN_WAYLAND" --xwayland --anland --exit-with-session="$SESSION_BIN"
EOS
EOF
RUN chmod +x /usr/local/bin/start-kde-anland && \
    ln -sf /usr/local/bin/start-kde-anland /usr/local/bin/start-kde-anland-direct

RUN cat <<'EOF' > /usr/local/bin/start-kde-anland-debug
#!/bin/bash
set -euo pipefail
...
KWIN_WAYLAND="$(command -v kwin_wayland 2>/dev/null || find /usr/bin /usr/lib /usr/libexec -type f -name kwin_wayland 2>/dev/null | head -n 1)"
if [ -z "$KWIN_WAYLAND" ]; then
    echo "ERROR: kwin_wayland not found"
    exit 1
fi
...
echo "anland socket: $SOCK"
echo "kwin wayland: $KWIN_WAYLAND"
exec dbus-run-session -- "$KWIN_WAYLAND" --xwayland --anland "$@"
EOF
RUN chmod +x /usr/local/bin/start-kde-anland-debug

RUN cat <<'EOF' > /opt/anland-kdefix/README.Droidspaces
Ubuntu-26-KDE-Anland starts KDE Plasma with patched kwin_wayland directly on the anland display daemon.

Default route:
  start-kde-anland [/run/display.sock]

Debug compositor-only route:
  start-kde-anland-debug [/run/display.sock]

This target intentionally does not start Weston. Ubuntu-26-KDE-Wayland remains the Weston nested fallback route.
EOF
